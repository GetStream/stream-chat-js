...
export class MessageComposer extends WithSubscriptions {
  ...
  get hasSendableData() {
    ...
    if (this.allowsPendingUploads) {
      ...
      // along on the optimistic message, where the user retries it. `blocked` still does not —
      // the server's upload configuration refused it, so no retry can settle it, and a message
      // whose only attachment was rejected outright must not look sendable.
