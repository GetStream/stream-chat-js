 ...
 * only attachments that already resolved to a URL. Whoever performs the send fills in the rest
 * once the uploads settle.
 *
 ...
 * optimistic message instead keeps the preview visible and leaves the `file` handle in reach of a
 * retry. `blocked` is the one state that stays behind - the server's upload configuration refused
 * it, so no retry can ever settle it, and it belongs in the composer where the user can remove it.
 */
const composeWithPendingUploads = ({
  ...
  // entry had already been cleaned up. The same collision is what a retry from two places
  // would cause, so both pending and failed uploads stay in the composer in that case and ride
  // along with the next message once they settle.
 ...
 * that.
 *
 * An attachment whose upload already `failed` is on `localMessage.attachments` too, and there is
 * nothing to await for it — the send half has to either retry it or send without it. Either way
 * the user can see it: `sanitizeOutgoingAttachments` drops any attachment still carrying
 * `localMetadata` with no remote URL, so a failed one cannot reach the API by accident.
 *
