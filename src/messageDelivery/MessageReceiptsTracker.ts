...
const toTimestampMs = (value: Date | string) => value instanceof Date ? value.getTime() : new Date(value).getTime();
...
export class MessageReceiptsTracker extends WithSubscriptions {
  ...
  constructor({ channel, locateMessage }: OwnMessageReceiptsTrackerOptions) {
    ...
    this.locateMessage = locateMessage ?? ((timestampMs: number) => {
  ...
  private upsertUserProgress(nextUserProgress: UserProgress) {
    ...
    const hasSameReadRef =
      compareRefsAsc(existingUserProgress.lastReadRef, nextUserProgress.lastReadRef) === 0 &&
