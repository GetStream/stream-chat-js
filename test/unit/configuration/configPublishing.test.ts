 ...
 * scalar.
 *
 * The dominant source was a repeated channel query. The API returns a **fresh** config object on each
 * response, so `_addChannelConfig` replaced the stored one, the by-cid selector in
 * `MessageComposer.subscribeChannelConfigChanged` fired, and the channel's composer re-resolved. Measured on
 * a 10-channel page with three open composers, back when the store was keyed by type and one write woke all
 * three: **30 publishes and 30 subscriber runs, down to 3** — one per composer, for the config genuinely
 * arriving the first time. Keying by cid narrows the fan-out further, but the guard is what makes a repeated
 * query free.
 *
...
describe('configuration publishes skip no-ops', () => {
  ...
  describe('at the source — client._addChannelConfig', () => {
    it('ignores a config deep-equal to the one already stored', () => {
      ...
      const first = client.channelServerConfigs['messaging:general'];
      ...
      expect(client.channelServerConfigs['messaging:general']).toBe(first);
    });
    ...
    it('does not notify the store for a repeated identical config', () => {
      ...
      client.channelServerConfigsStore.subscribe(listener);
      listener.mockClear();
    ...
    it('still stores a config that genuinely changed', () => {
      ...
      expect(client.channelServerConfigs['messaging:general']).toMatchObject({
    ...
    it('keeps a repeated channel query from waking live composers', () => {
      ...
      // The config arrives for the first time: each composer should hear about its own channel's.
