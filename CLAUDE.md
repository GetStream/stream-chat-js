...
- **`connection_id_manager.ts` — `ConnectionIdManager`.** Holds the WS connection id (`client.connectionIdManager`, read through `client._getConnectionID()`) and lets callers await one that is still being negotiated. The server keys channel watches and presence subscriptions by connection id and answers `200` while registering nothing when a request that needs one arrives without it, so `ApiClient._doRequest` gates every such request on `getConnectionId()` and attaches the id — `requiresConnectionId` (`api-client.ts`) decides which, from the `watch`/`presence` flags, falling back to a spec-declared `connection_id` param for `stopWatchingChannel` and `longPoll`, which carry no flag. No other request carries the id. `StableWSConnection` drives the lifecycle: `arm()` before a socket opens, `resolveConnectionId()` on the `connection.ok` frame, `invalidate()` from `_setHealth(false)` (drops the dead id and arms a replacement, so requests wait for the reconnect instead of racing ahead with it), `reset()` from `disconnect()` (drops and **rejects**, since no reconnect is coming). A reconnect that gives up rejects too — leaving the deferred armed would hang every gated request. `test/unit/codegen/connectionIdEndpoints.test.ts` pins the generated endpoint set the fallback depends on.
