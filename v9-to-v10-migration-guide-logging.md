...
The SDK routed _all_ internal log calls into this single function; disambiguation was done via the `extraData.tags` array (values like `'api'`, `'api_request'`, `'api_response'`, `'client'`, `'channel'`, `'connection'`, `'event'`).
...
- Configuration is not additive across calls to `configureLoggers` for the _same_ key — the last call wins per (scope, field). Untouched scopes keep their prior override.
...
8. **Remove `_log()` calls in any code that subclassed SDK internals.** Every `_log(msg, extra?, level?)` in v9 mapped to `this.client.logger(level ?? 'info', '<prefix>:' + msg, { tags: […], ...extra })`. Replace with the appropriate scoped logger. Example (from `StableWSConnection`):
   ...
   logger
     ...
     .info(`Established a WebSocket connection. Health check: ${hc}.`);
   ```
